...
function initializeMultipleForm(containerId, apiData, title, apiPath) {
    ...
    const displayHtml = buildMultipleItemsDisplay(apiData?.data || {}, title);
    container.innerHTML = displayHtml;
    
    // Attach event listeners
    attachMultipleItemsEventListeners();
}
 ...
 */
function buildMultipleItemsDisplay(itemsData, title) {
    const itemsList = Object.entries(itemsData || {}).map(([itemKey, itemData]) => {
        return buildSingleItemDisplay(itemKey, itemData);
    }).join('');
 ...
 */
function buildSingleItemDisplay(itemKey, itemData) {
 ...
 */
function attachMultipleItemsEventListeners() {
    // Add new item button
    ...
    if (saveItemsBtn) {
        saveItemsBtn.addEventListener('click', saveAllItems);
    }
    ...
    deleteButtons.forEach(btn => {
        btn.addEventListener('click', (event) => {
            ...
            deleteItem(event.target.dataset.itemKey);
        });
    });
    ...
    editButtons.forEach(btn => {
        btn.addEventListener('click', (event) => {
            event.stopPropagation(); // Prevent card click
            toggleEditMode(event.target.dataset.itemKey);
        });
    });
...
function addNewItem() {
    ...
    const newItemHtml = buildSingleItemDisplay(key, newItemData);
 ...
 */
function toggleEditMode(itemKey) {
    const itemCard = document.querySelector(`[data-item-key="${itemKey}"]`);
    if (!itemCard) return;
 ...
 */
function saveItemChanges(itemKey) {
    const itemCard = document.querySelector(`[data-item-key="${itemKey}"]`);
    if (!itemCard) return;
...
async function saveAllItems() {
    ...
    try {
        ...
        const itemsData = collectAllItemsData();
